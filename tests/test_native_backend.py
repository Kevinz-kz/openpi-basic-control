...
def test_native_backend_forwards_franka_hand_arguments(monkeypatch) -> None:
    captured_args: list[str] = []
    backend = NativeArmBackend()
    config = ArmConfig(
        "fr3-follower",
        ...
        effector_connection=FrankaHandConnection(speed_m_s=0.08),
    )
    ...
    try:
        with pytest.raises(RuntimeError, match="captured native arguments"):
            backend.connect(
                config,
                ArmRole.FOLLOWER,
                topics_for("fr3-hand", "fr3-follower"),
            )
    finally:
        backend.close()
    ...
    # The hand's address defaults to the arm's own controller.
    assert value_after("--franka_hand_address") == "192.168.1.10"
    assert value_after("--franka_hand_speed") == "0.08"
    assert value_after("--franka_hand_homing") == "false"
    assert value_after("--effector_model") == "Franka_hand"
    assert not [argument for argument in captured_args if argument.startswith("--robotiq")]
